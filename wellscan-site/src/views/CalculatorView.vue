        ...
        <form method="POST" action="#" id="calc-form" class="calc-form">
            <div class="calc-sec">
                ...
                <div class="row">
                    ...
                    <div class="radio">
                        <input v-model="category" type="radio" name='foodType' id="processedPackage"
            ...
            <div class="calc-sec">
                ...
                <div class="nutrition-div">
                    <p>Saturated Fat (g)</p>
                    <div class="textbox">
                        <input v-model="satFat" pattern="[0-9]*" placeholder="Enter amount" inputmode="numeric"
                            type="number" name='satfat' id="satfat">
                    </div>
                </div>
                ...
                <div class="nutrition-div">
                    <p>Sodium (mg)</p>
                    <div class="textbox">
                        <input v-model="sodium" pattern="[0-9]*" placeholder="Enter amount" inputmode="numeric"
                            type="number" name='sodium' id="sodium">
                    </div>
                </div>

                <div class="nutrition-div">
                    <p><select v-model="sugarType" id="sugarType">
            ...
            <button @click.prevent="handleCalculate" class="btn btn-block btn-primary" type="submit">calculate</button>
            <button @click.prevent="handleReset" id="btn_reset" class="btn btn-block btn-outline-secondary" type='reset'>reset</button>
        </form>
    </section>
</template>
...
const handleCalculate = async () => {
    if (satFat.value === null || sodium.value === null || sugar.value === null || category.value === "") {
        return;
    }
    console.log("Submitted");
    let sugars: number = 0, added_sugars: number = 0;
    if (sugarType.value === "added") {
        added_sugars = sugar.value;
    } else 
    if (sugarType.value === "total") {
        sugars = sugar.value;
    }

    let endpoint: string = "https://v2.api.wellscan.io/api/foods/rankFromNuts/" + category.value + "/" + satFat.value + "/" + sodium.value + "/" + added_sugars + "/" + sugars;
    console.log(endpoint);
    const response: AxiosResponse | void = await axios.get(endpoint).catch((error: AxiosError) => {
        console.log(error);
        return;
    });
    if (response) {
